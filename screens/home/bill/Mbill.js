...
const Mbill = ({ navigation, route }) => {
    const romes = [{ name: '101', people: 1, status: false, cost: '6.245.000' },
    ]
                ...
                {ds.map((value, index) => {
                    return <Pressable key={index} onPress={() => {
                        ...
                        <View style={styles.dsdv} >
                            ...
                            <View>
                                {value.status ? <Text style={{ fontSize: 16, color: 'green' }}>Đã đóng</Text> :
                                    <Text style={{ fontSize: 16, color: 'red' }}>Chưa đóng</Text>}
